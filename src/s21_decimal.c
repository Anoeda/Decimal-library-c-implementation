...
int s21_add(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  /* Функция для суммы 2х значений s21_decimal */
  ...
  !errno ? convert_to_s21_decimal(&b_result, result)
         : reset_s21_decimal(result);

  return errno;
}

int s21_sub(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  /* Функция для вычитания 2х значений s21_decimal */

  s21_negate(value_2, &value_2);
  errno = s21_add(value_1, value_2, result);

  return errno;
}

int s21_mul(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  /* Функция для умножения 2х значений s21_decimal */
...
int s21_div(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  /* Функция для деления 2х значений s21_decimal */
...
int s21_mod(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  /* Функция для взятия остатка 2х значений s21_decimal */
...
void big_fractional_division(big_decimal *b_value_1, big_decimal *b_value_2,
  ...
  while (!is_integer_division && get_big_decimal_scale(b_result) < MAX_SCALE) {
    big_decimal t_b_result = {0};
...
void convert_to_big_decimal(const s21_decimal *value, big_decimal *b_value) {
  /* Функция для конвертации s21_decimal --> big_decimal */
...
void convert_to_s21_decimal(const big_decimal *b_result, s21_decimal *result) {
  /* Функция для конвертации big_decimal --> s21_decimal */
...
void reset_s21_decimal(s21_decimal *value) {
  /* Функция для обнуления значений всех битовых векторов значения s21_decimal
   */
...
int error_processing(const big_decimal *b_value) {
  /* Функция для обработки ошибок в арифметических функциях s21_decimal */
...
int is_decimal_overflow(const big_decimal *b_value) {
  /* Функция для проверки переполнения значения s21_decimal, записанного
     в big_decimal */
...
int s21_is_less(s21_decimal value_1, s21_decimal value_2) {
  /* Функция для определения, является ли значение s21_decimal value_1 меньше
   * value_2 */
...
int s21_is_less_or_equal(s21_decimal value_1, s21_decimal value_2) {
  /* Функция для определения, является ли значение s21_decimal value_1 меньшим
     либо равным value_2 */
...
int s21_is_greater(s21_decimal value_1, s21_decimal value_2) {
  /* Функция для определения, является ли значение s21_decimal value_1 больше
   * value_2 */
  ...
  int are_values_zero =
          is_s21_decimal_empty(&value_1) && is_s21_decimal_empty(&value_2),
...
int s21_is_greater_or_equal(s21_decimal value_1, s21_decimal value_2) {
  /* Функция для определения, является ли значение s21_decimal value_1 большим
     либо равным value_2 */
...
int s21_is_equal(s21_decimal value_1, s21_decimal value_2) {
  /* Функция для определения равенства 2х значений s21_decimal */
...
int s21_is_not_equal(s21_decimal value_1, s21_decimal value_2) {
  /* Функция для определения неравенства 2х значений s21_decimal */
...
int is_s21_decimal_empty(const s21_decimal *value) {
  ...
  int is_s21_decimal_empty = 1;

  for (int n_bit_vector = 0;
       is_s21_decimal_empty && n_bit_vector < BIT_VECTORS_NUM; n_bit_vector++) {
    if (value->bits[n_bit_vector]) {
      is_s21_decimal_empty -= 1;
    }
  }

  return is_s21_decimal_empty;
}

// CONVERTERS
int s21_from_int_to_decimal(int src, s21_decimal *dst) {
  int flag = 0;
  s21_decimal_to_default(dst);
  if (src < 0) {
    setSignNegative(dst);
    dst->bits[0] = -src;
  } else {
...
int s21_from_float_to_decimal(float src, s21_decimal *dst) {
  ...
  if ((src < 1e-28 && src > 0) || (src > -1e-28 && src < 0)) {
    ...
    dst->bits[1] = s21_ZERO;
    dst->bits[2] = s21_ZERO;
    dst->bits[3] = minus ? s21_NEGATIVE_ZERO : s21_ZERO;
    flag = 1;
  } else if (binexp > -95) {
    int scale = 0;
    double result = 1.0;
    ...
    for (; scale + 7 <= 0; s21_shift_left_scale(dst, 1), scale++) {
    }
...
int s21_from_decimal_to_int(s21_decimal src, int *dst) {
  int flag = 0;
  if ((src.bits[3] != s21_INFINITY) && (src.bits[3] != s21_NEGATIVE_INFINITY) &&
      dst) {
    if (getSign(&src)) {
      setSignZero(&src);
      src = s21_TruncateScale(src);
      setSignNegative(&src);
    } else {
      src = s21_TruncateScale(src);
    }
...
void clearScale(s21_decimal *current) { current->bits[3] = 0x00000000; }
...
int getSign(s21_decimal *current) { return current->bits[3] >> 31; }

int getScale(s21_decimal *current) { return (current->bits[3] >> 16); }
...
int s21_round(s21_decimal value, s21_decimal *result) {
  ...
  if (value.bits[BIT_VECTORS_NUM] > SCALE_28) {
    ...
    if (value.bits[BIT_VECTORS_NUM] > 0) {
      s21_decimal five = {{5, 0, 0, SCALE_1}};
      s21_decimal temp = {{0}};

      s21_add(value, five, &temp);
      value = temp;
      s21_truncate(value, result);
    } else {
      *result = value;
...
int s21_truncate(s21_decimal value, s21_decimal *result) {
  ...
  if (value.bits[BIT_VECTORS_NUM] > SCALE_28) {
    ...
    if (value.bits[BIT_VECTORS_NUM] > 0) {
      ...
      convert_to_s21_decimal(&b_value, &value);
    }
    *result = value;
    if (minus) {
      result->bits[BIT_VECTORS_NUM] =
